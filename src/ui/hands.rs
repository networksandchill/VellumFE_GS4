...
impl Hands {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        if only_horizontal_borders {
            // For left/right only borders, we'll manually render them on each content row
        ...
        // If we have left/right only borders, render them manually on each content row
        if only_horizontal_borders {
            let has_left = borders.contains(ratatui::widgets::Borders::LEFT);
            let has_right = borders.contains(ratatui::widgets::Borders::RIGHT);

            // Render borders on all 3 hand rows
            for row in 0..3 {
                if row >= inner_area.height {
                    break;
                }
