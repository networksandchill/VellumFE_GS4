...
impl TuiFrontend {
    ...
    fn handle_mouse_scroll(
        ...
        // A map surface zooms instead of scrolling.
        if self.map_zoom_at(app_core, &target_window, x, y, if delta > 0 { 1 } else { -1 }) {
            return Ok((true, None));
        }
    ...
    fn handle_mouse_down_right(
        ...
        // Right-click on a map surface: room-specific menu (travel, set
        // destination, ...) instead of the generic window menu.
        let target = find_topmost_window_at(&app_core.ui_state, x, y);
        let is_map_surface = match app_core.ui_state.get_window(&target).map(|w| &w.content) {
            ...
            if let Some(pane) = self.widget_manager.map_panes.get(&target) {
                if pane.contains(x, y) {
                    if let Some(room_id) = pane.room_at(x, y) {
                        let items = Self::build_map_room_menu(app_core, room_id);
                        app_core.ui_state.popup_menu =
                            Some(crate::data::ui_state::PopupMenu::new(items, (x, y + 1)));
                        app_core.ui_state.input_mode = InputMode::Menu;
                        app_core.needs_render = true;
        ...
        if let Some(name) = self.map_drag_window.take() {
            if let Some(pane) = self.widget_manager.map_panes.get_mut(&name) {
                if let Some(moved) = pane.end_drag() {
                    ...
                    if !moved {
                        if let Some(room_id) = pane.room_at(x, y) {
                            tracing::info!("Map click on room {room_id} in '{name}' -> .go2");
                            let cmd =
                                self.handle_command_submission(format!(".go2 {room_id}"), app_core)?;
                            return Ok((true, cmd));
                        }
                    }
                    return Ok((true, None));
    ...
    fn handle_mouse_drag_left(
        ...
        // A click-hold started on a map pane pans it, not the window.
    ...
    fn handle_mouse_down_left(
        ...
        if let (Some(window_name), Some(operation)) = (found_window.clone(), drag_op.clone()) {
            ...
            // Map surface (standalone map window, or the active map tab of a
            // tabbed window): click a room to travel there via the native walk
            // executor. Click-hold drags pan the map; the click-vs-drag call is
            // made on release.
            let is_map_surface =
                match app_core.ui_state.get_window(&window_name).map(|w| &w.content) {
                    Some(crate::data::WindowContent::Map(_)) => true,
                    Some(crate::data::WindowContent::TabbedText(_)) => self
                        .widget_manager
                        .tabbed_text_windows
            ...
            if is_map_surface {
                if let Some(pane) = self.widget_manager.map_panes.get_mut(&window_name) {
                    if pane.contains(x, y) {
                        pane.begin_drag(x, y);
